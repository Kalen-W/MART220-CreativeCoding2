...
function renderScene(rays) {
  ...
  rays.forEach((ray, i) => {
    ...
    const wallHeight = (viewDist * cellSize) / dist;
    ...
    var sx = 0;
    var sy = 0;
    var top = Math.round( ((screenH + screenHMod) - wallHeight) / 2 );
...
function renderFloor() {
  var floorGradient = context.createLinearGradient(0,screenH, 0,-center.y + screenHMod/2);
  floorGradient.addColorStop(0, colors.floorBottom);
  floorGradient.addColorStop(1, colors.floorTop);
  context.fillStyle = floorGradient;
  //context.fillRect(0,center.y + screenHMod/2, screenW,center.y - screenHMod/2);
  context.fillRect(0,screenH, screenW,-center.y + screenHMod/2);
}

function renderSky() {
  var ceilingGradient = context.createLinearGradient(0,0, 0,center.y + screenHMod/2);
  ceilingGradient.addColorStop(0, colors.ceilingTop);
  ceilingGradient.addColorStop(1, colors.ceilingBottom);
  context.fillStyle = ceilingGradient;
  context.fillRect(0,0, screenW,center.y + screenHMod/2);
}


//--------------------------------------------------------------------------------------------------------------------------------|Sprite Rendering
/*function renderSprites0() {
  ...
  for (var i=0; i<visibleSprites.length; i++) {
    ...
    var size = (viewDist * cellSize) / (Math.cos(spriteAngle) * dist); // Both size expressions here seem to give the same or very similar results.
    ...
    img.style.left = Math.round(center.x + x - size / 2) + "px";
    img.style.top = Math.round((screenH - size) / 2) + "px";
  ...
  //console.log(oldVisibleSprites.length);
...
function renderSprites(rays) {
  ...
  for (var i=0; i<visibleSprites.length; i++) {
    ...
    var top = Math.round( ((screenH + screenHMod) - size) / 2 );
...
function renderEnemies(rays) {
  ...
  for (var i=0; i<enemies.length; i++) {
    ...
    if (angle < -fov*0.55 || angle > fov*0.55) { continue; }
  ...
  //sortEnemies(enemiesInCrosshair);
  //console.log(enemiesInCrosshair);
}
