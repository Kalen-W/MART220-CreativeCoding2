...
function renderLoop(timeStamp) {
  ...
  if (!pause) {
    ...
    // Particle creation - for testing purposes.
    //if (lmbIsDown) { createOverlayParticles(mousePos.x, mousePos.y, 4, 'smoke'); }
  }
  ...
  // Once the pause menu is created with HTML elements, I shouldn't need to have these stated twice.
  // updateOverlayParticles();
  // updateWorldParticles();
  //
  // player.heldItem.render(overlayContext)
  ...
  // I can't tell for certain, but it feels like requestAnimationFrame is
  // less smooth than setTimeout, even if it gets a higher frame rate.
  ...
  // context.putImageData(wallsImgData, 0, 0);
}
...
function pauseUpdate(override) {
  ...
  if (pause) {
    ...
    if (!options.mobileControls) { overlayCanvas.requestPointerLock(); }
