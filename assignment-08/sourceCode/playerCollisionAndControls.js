...
class PlayerClass {
  ...
  movement(timeDelta) {
    ...
    if (keyMap.move_jump && this.jumpHeight <= 0) {
      this.jump();
    }
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key == 't') {
    createWorldParticles(15.5*cellSize, 24.5*cellSize, 1, 8, 'gibs');
    console.log(zBuffer);
    console.log(screenImgData);
    //console.log(wallsImgData.data[98336]);
  }
});
...
onkeydown = onkeyup = function(e){
  // Failed / unused attampts
  /*keyMap[e.key.toLowerCase()] = e.type == 'keydown';
  if (e.key == controlKeys.move_sprint) {
    keyMap[controlDefaults.move_sprint.toLowerCase()] = e.type == 'keydown';
  }
  if (e.key == controlKeys.move_sprint) {
    keyMap.move_sprint = e.type == 'keydown';
    console.log("test");
  }*/
